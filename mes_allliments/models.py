from django.db import models


def categorie(models.Model):
    name_categorie = models.CharField(max_length=60)
    


def aliment(models.Model):
    name_aliment = models.CharField(max_length=60)
    code_product_food = models.CharField(max_length=20)
    description = models.TextField()
    nutriscore = models.CharField(max_length=1)
    image = models.ImageField()

    id_categorie = models.ForeignKey(categorie, on_delete=models.CASCADE)


def store(models.Model):
    name_store = models.CharField(max_length=60)
    code_product_store = models.CharField(max_length=60)

    aliment = models.ForeignKey(aliment, on_delete=models.CASCADE


def brand(models.Model):

    name_store = models.CharField(max_length=60)
    code_product_brands = models.CharField(max_length=60)
                                
    aliment = models.ForeignKey(aliment, on_delete=models.CASCADE





